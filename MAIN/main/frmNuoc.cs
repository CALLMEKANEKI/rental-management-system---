...
namespace MAIN.main
{
    ...
    {
        private nuocService nuocService = new nuocService();
        ...
        private void LoadDGVNuoc()
        {
            try
            {
                List<nuocViewModel> listNuoc = nuocService.LayTatCaBanGhiNuocViewModel(id_chutrohientai);
        ...
        public void EditDGVNuoc ()
        {
            dgvNuoc.Columns["IdNuoc"].HeaderText = "Mã bản ghi nước";
            dgvNuoc.Columns["IdNuoc"].DisplayIndex = 0;

            dgvNuoc.Columns["NgayTao"].HeaderText = "Ngày tạo";
            dgvNuoc.Columns["NgayTao"].DisplayIndex = 1;

            dgvNuoc.Columns["TenPhong"].HeaderText = "Phòng";
            dgvNuoc.Columns["TenPhong"].DisplayIndex = 2;

            dgvNuoc.Columns["ChiSo_Dau"].HeaderText = "Chỉ số cũ";
            dgvNuoc.Columns["ChiSo_Dau"].DisplayIndex = 3;

            dgvNuoc.Columns["ChiSo_Cuoi"].HeaderText = "Chỉ số mới";
            dgvNuoc.Columns["ChiSo_Cuoi"].DisplayIndex = 4;

            dgvNuoc.Columns["TienNuoc"].HeaderText = "Tiền nước";
            dgvNuoc.Columns["TienNuoc"].DisplayIndex = 5;
            dgvNuoc.Columns["TienNuoc"].DefaultCellStyle.Format = "N0";

            dgvNuoc.Columns["TrangThai"].HeaderText = "Trạng thái";
            dgvNuoc.Columns["TrangThai"].DisplayIndex = 6;

            // Cho phép chọn toàn bộ hàng
            dgvNuoc .SelectionMode = DataGridViewSelectionMode.FullRowSelect;
        }

        public void ClearDataNuoc()
        {
            txtMaNuoc.Text = "";
            txtChiSoDau.Text = "";
            txtChiSoCuoi.Text = "";
        ...
        public void DGVNuoc_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txtMaNuoc.Text = GetValue("IdNuoc")?.ToString() ?? string.Empty;
                ...
                txtTienNuoc.Text = GetValue("TienNuoc")?.ToString() ?? "0";
                txtPhong.Text = GetValue("TenPhong")?.ToString() ?? "0";
                txtTrangThai.Text = GetValue("TrangThai")?.ToString() ?? "0";
            }
        }

        private void txtTimNuoc_Enter(object sender, EventArgs e)
        {
            if (txtTimNuoc.Text == "Nhập từ khóa để tìm kiếm")
            {
                txtTimNuoc.Text = "";
                txtTimNuoc.ForeColor = System.Drawing.Color.Black;
            }
        }

        private void txtTimNuoc_Leave(object sender, EventArgs e)
        {
            if (txtTimNuoc.Text == "")
            {
                txtTimNuoc.Text = "Nhập từ khóa để tìm kiếm";
                txtTimNuoc.ForeColor = System.Drawing.Color.Gray;
            }
        }
        ...
        private void btnTimNuoc_Click(object sender, EventArgs e)
        {
            ...
            List<nuocViewModel> searchResults = nuocService.LayTatCaBanGhiNuocViewModelTheoKeyWork(id_chutrohientai, keyword);

            // 3. Xử lý kết quả tìm kiếm
            if (searchResults == null || searchResults.Count == 0)
            {
                MessageBox.Show("Không tìm thấy bản ghi nước nào phù hợp với từ khóa: " + keyword, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            // 4. Load DataGridView với kết quả mới
            dgvNuoc.DataSource = searchResults;
            EditDGVNuoc();
        }
        ...
        private void btnXuat_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    // Tạo tiêu đề
                    worksheet.Cells[1, 1].Value = "DANH SÁCH BẢN GHI NƯỚC";
                    ...
                    worksheet.Cells[3, 2].Value = dgvNuoc.Rows.Count;
                    ...
                    worksheet.Cells[startRow, 6].Value = "Tiền Nước";
                    ...
                    for (int i = 0; i < dgvNuoc.Rows.Count; i++)
                    {
                        ...
                        worksheet.Cells[row, 1].Value = dgvNuoc.Rows[i].Cells["IdNuoc"].Value?.ToString();

                        // Định dạng ngày tháng
                        var ngayTao = dgvNuoc.Rows[i].Cells["NgayTao"].Value;
                        ...
                        worksheet.Cells[row, 3].Value = dgvNuoc.Rows[i].Cells["TenPhong"].Value?.ToString();
                        worksheet.Cells[row, 4].Value = dgvNuoc.Rows[i].Cells["TrangThai"].Value?.ToString();
                        // Định dạng các cột tiền
                        FormatCurrencyCell(worksheet, row, 5, dgvNuoc.Rows[i].Cells["ChiSo_Dau"].Value); // Tiền Phòng
                        FormatCurrencyCell(worksheet, row, 6, dgvNuoc.Rows[i].Cells["ChiSo_Cuoi"].Value); // Tiền Điện
                        FormatCurrencyCell(worksheet, row, 7, dgvNuoc.Rows[i].Cells["TienNuoc"].Value); // Tiền Nước
                    }
                    ...
                    int endRow = startRow + dgvNuoc.Rows.Count;
                    ...
                    for (int i = 0; i < dgvNuoc.Rows.Count; i++)
                    {
                        var thanhTienValue = dgvNuoc.Rows[i].Cells["TienDien"].Value;
                    ...
                    {
                        var thanhTienValue = dgvNuoc.Rows[i].Cells["TienDien"].Value;
                        var trangthai = dgvNuoc.Rows[i].Cells["TrangThai"].Value?.ToString();
                    ...
                    var thongKeTrangThai = dgvNuoc.Rows.Cast<DataGridViewRow>()
                    ...
                    saveFileDialog.FileName = $"DanhSachBanGhiNuoc_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
                    ...
                    if (saveFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        MessageBox.Show($"Xuất Excel thành công!\nTổng số bản ghi nước: {dgvNuoc.Rows.Count}\nTổng doanh thu: {tongTien:N0} VNĐ",
